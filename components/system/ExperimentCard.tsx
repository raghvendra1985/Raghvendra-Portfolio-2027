  ...
  return (
    <article data-os-item className="border-t border-line py-8">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <p className="max-w-2xl type-h3">{item.hypothesis}</p>
        <p className="font-mono-label text-green">{item.status}</p>
      </div>
      <dl className="mt-6 grid gap-6 sm:grid-cols-2">
        <div>
          <dt className="font-mono-label text-ink-soft">What is being tested</dt>
          <dd className="mt-2 text-sm leading-relaxed text-ink-soft">{item.testing}</dd>
        </div>
        <div>
          <dt className="font-mono-label text-ink-soft">Learning</dt>
