...
export default function RelatedReading({ articles }: { articles: KnowledgeArticle[] }) {
  ...
  return (
    <nav aria-label="Related reading" className="border-t border-line pt-12">
      <h2 className="font-mono-label text-[11px] text-ink-soft">Related reading</h2>
      <ul className="mt-8">
        {articles.map((article) => (
          <li key={article.slug} data-related-item>
            ...
            >
              <p className="font-mono-label text-[11px] text-gold">{article.category}</p>
              <div>
                <p className="font-display text-2xl sm:text-3xl">{article.title}</p>
