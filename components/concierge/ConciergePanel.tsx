...
export default function ConciergePanel() {
  ...
  const runQuery = (value: string) => {
    ...
    setAnswer(result);
    setActiveIndex(-1);
    trackConcierge("concierge_query", { query: trimmed, mode, results: result.evidence.length });
    if (result.noResult) {
  ...
  return (
    <div ref={rootRef} className="fixed inset-0 z-[80]">
      <button
        ...
        aria-label="Close Ask Raghvendra"
      ...
      >
        <div className="flex items-start justify-between gap-6">
          <div>
            <p className="font-mono-label text-[11px] text-gold">Ask Raghvendra</p>
        ...
        <p className="mt-10 font-mono-label text-[10px] text-ink-soft" aria-hidden={optionCount < 0}>
